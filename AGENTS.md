...
- 中断（OOM kill・再起動・メモリ都合の一時停止）は自動で last checkpoint から再開する。
  マシンが**丸ごと落ちる**（`dmesg` に "Out of memory"）なら学習設定でなく OS の問題 →
  [docs/stability.md](docs/stability.md) の cgroup 上限・swap・earlyoom を設定する（GB10 は特に）。
- **GPU が他で塞がっている / 3回リトライしても失敗** → 止まって [⚑ HUMAN] に状況を伝える。
  （メモリ都合の一時停止はリトライ回数を消費しないので、3回失敗は本物の失敗。）
